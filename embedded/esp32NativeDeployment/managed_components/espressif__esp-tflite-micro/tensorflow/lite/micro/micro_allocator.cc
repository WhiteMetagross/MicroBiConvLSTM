...
TfLiteStatus InitializeTfLiteTensorFromFlatbuffer(
  ...
  *result = {};
  ...
  if (src_quantization && src_quantization->scale() &&
    ...
    bool has_uniform_zero_points = true;
    const int64_t first_zero_point = src_quantization->zero_point()->Get(0);
    for (size_t i = 1; i < src_quantization->zero_point()->size(); ++i) {
      if (src_quantization->zero_point()->Get(i) != first_zero_point) {
        has_uniform_zero_points = false;
        break;
      }
    }
    const bool compact_zero_points =
        src_quantization->zero_point()->size() == 1 || has_uniform_zero_points;
    const int zero_point_count = compact_zero_points ? 1 : channels;

    // Preserve the compact single-value zero-point form when the flatbuffer
    // already uses it. This avoids needlessly expanding per-channel weight
    // metadata into large temporary arrays during AllocateTensors().
    quantization->zero_point =
        allocate_temp
            ? reinterpret_cast<TfLiteIntArray*>(
                  non_persistent_buffer_allocator->AllocateTemp(
                      TfLiteIntArrayGetSizeInBytes(zero_point_count),
    ...
    quantization->zero_point->size = zero_point_count;
    int* zero_point_data = quantization->zero_point->data;
    for (int i = 0; i < zero_point_count; i++) {
      zero_point_data[i] = compact_zero_points
                               ? static_cast<int>(first_zero_point)
...
TfLiteStatus MicroAllocator::CommitStaticMemoryPlan(
  ...
  size_t planner_scratch_size = remaining_arena_size;
  size_t minimum_planner_scratch_size = remaining_arena_size;
  if (allocation_info_count > 0) {
    const size_t greedy_bytes =
        static_cast<size_t>(allocation_info_count) *
        GreedyMemoryPlanner::per_buffer_size();
    if (greedy_bytes > minimum_planner_scratch_size) {
      minimum_planner_scratch_size = greedy_bytes;
    }
    if (greedy_bytes > planner_scratch_size) {
      planner_scratch_size = greedy_bytes;
    }
  }
  if (planner_scratch_size < 64 * 1024) {
    planner_scratch_size = 64 * 1024;
  }
  uint8_t* planner_arena = nullptr;
  size_t heap_planner_scratch_size = planner_scratch_size;
  while (heap_planner_scratch_size >= minimum_planner_scratch_size) {
    planner_arena =
        reinterpret_cast<uint8_t*>(std::malloc(heap_planner_scratch_size));
    if (planner_arena != nullptr) {
      break;
    }
    if (heap_planner_scratch_size == minimum_planner_scratch_size) {
      break;
    }
    const size_t next_candidate =
        heap_planner_scratch_size > (16 * 1024)
            ? heap_planner_scratch_size - (16 * 1024)
            : minimum_planner_scratch_size;
    heap_planner_scratch_size =
        next_candidate < minimum_planner_scratch_size
            ? minimum_planner_scratch_size
            : next_candidate;
  }
  const bool planner_arena_on_heap = planner_arena != nullptr;
  if (!planner_arena_on_heap) {
    planner_arena = non_persistent_buffer_allocator_->AllocateTemp(
        remaining_arena_size, MicroArenaBufferAlignment());
    planner_scratch_size = remaining_arena_size;
  } else {
    planner_scratch_size = heap_planner_scratch_size;
  }
  MicroPrintf("Planner scratch bytes=%d heap_backed=%d minimum_bytes=%d",
              static_cast<int>(planner_scratch_size),
              planner_arena_on_heap ? 1 : 0,
              static_cast<int>(minimum_planner_scratch_size));

  if (planner_arena == nullptr) {
    return kTfLiteError;
  }

  memory_planner_->Init(planner_arena,
                        static_cast<int>(planner_scratch_size));
  TF_LITE_ENSURE_STATUS(

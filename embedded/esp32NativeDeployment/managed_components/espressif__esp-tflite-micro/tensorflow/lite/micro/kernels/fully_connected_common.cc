...
TfLiteStatus CalculateOpDataFullyConnected(
  ...
  int filter_zero_point = filter->params.zero_point;
  ...
  if (filter->quantization.type == kTfLiteAffineQuantization &&
    ...
    if (affine_quantization == nullptr) {
      MicroPrintf("FC prepare: affine_quantization is null.");
      return kTfLiteError;
    }
    if (affine_quantization->scale == nullptr) {
      MicroPrintf("FC prepare: affine_quantization->scale is null.");
      return kTfLiteError;
    }
    if (affine_quantization->zero_point != nullptr &&
        affine_quantization->zero_point->size > 0) {
      filter_zero_point = affine_quantization->zero_point->data[0];
      for (int i = 1; i < affine_quantization->zero_point->size; ++i) {
        if (affine_quantization->zero_point->data[i] != filter_zero_point) {
          MicroPrintf(
              "FC prepare: non-uniform per-channel zero_point[%d]=%d first=%d",
              i, affine_quantization->zero_point->data[i], filter_zero_point);
          return kTfLiteError;
        }
      }
    }
...
// code, and only reference the data->is_per_channel in non-HEXAGON code.
    ...
    if (!((input->type == kTfLiteInt8 && filter->type != kTfLiteInt4) ||
      ...
      MicroPrintf("FC prepare: unsupported per-channel types input=%s filter=%s",
                  TfLiteTypeGetName(input->type),
                  TfLiteTypeGetName(filter->type));
      return kTfLiteError;
    }

    if (affine_quantization->scale->size != per_channel_quantization_size) {
      MicroPrintf(
          "FC prepare: scale size mismatch scale_size=%d expected=%d",
          affine_quantization->scale->size, per_channel_quantization_size);
      return kTfLiteError;
    }

    const int quantized_dimension = affine_quantization->quantized_dimension;
    if (quantized_dimension < 0 || quantized_dimension >= filter->dims->size) {
      MicroPrintf(
          "FC prepare: invalid quantized_dimension=%d dims_size=%d",
          quantized_dimension, filter->dims->size);
      return kTfLiteError;
    }
    if (per_channel_quantization_size !=
        filter->dims->data[quantized_dimension]) {
      MicroPrintf(
          "FC prepare: quantized dimension mismatch size=%d dims[%d]=%d",
          per_channel_quantization_size, quantized_dimension,
          filter->dims->data[quantized_dimension]);
      return kTfLiteError;
    }
    ...
    data->per_channel_scales = affine_quantization->scale->data;
    data->per_channel_quantization_size = per_channel_quantization_size;
    data->input_scale = input->params.scale;
    data->output_scale = output->params.scale;
#endif
  } else {
    double real_multiplier = 0.0;
    const TfLiteStatus multiplier_status = GetQuantizedConvolutionMultipler(
        context, input, filter, bias, &output_snapshot, &real_multiplier);
    if (multiplier_status != kTfLiteOk) {
      MicroPrintf("FC prepare: GetQuantizedConvolutionMultipler failed.");
  ...
  // context.
  if (filter_zero_point != 0) {
    MicroPrintf("FC prepare: filter zero_point=%d (expected 0)",
                filter_zero_point);
  }

  data->input_zero_point = input->params.zero_point;
  data->filter_zero_point = filter_zero_point;
  data->output_zero_point = output_snapshot.params.zero_point;

  return CalculateActivationRangeQuantized(context, activation, &output_snapshot,

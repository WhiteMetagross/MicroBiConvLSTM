...
TfLiteStatus Prepare(TfLiteContext* context, TfLiteNode* node) {
  ...
  if (input->type == kTfLiteInt8) {
    ...
    TF_LITE_ENSURE(
        context, affine_quantization->zero_point->size == 1 ||
  ...
  if (input->type == kTfLiteInt8) {
    ...
    if (scratch_buf_size > 0) {
      if (context->RequestScratchBufferInArena(
              context, scratch_buf_size, &data->buffer_idx) == kTfLiteOk) {
        data->use_esp_nn = true;
      } else {
        data->buffer_idx = -1;
      }
    } else {

...
TfLiteStatus FullyConnectedPrepare(TfLiteContext* context, TfLiteNode* node) {
  ...
  if (input == nullptr) {
    MicroPrintf("FC prepare: input tensor is null.");
    return kTfLiteError;
  }
  TfLiteTensor* filter = micro_context->AllocateTempInputTensor(
      node, kFullyConnectedWeightsTensor);
  if (filter == nullptr) {
    MicroPrintf("FC prepare: filter tensor is null.");
    return kTfLiteError;
  }
  TfLiteTensor* bias =
  ...
  if (output == nullptr) {
    MicroPrintf("FC prepare: output tensor is null.");
    return kTfLiteError;
  }
  if (input->type != output->type) {
    MicroPrintf("FC prepare: input/output type mismatch input=%s output=%s",
  ...
  // Release temp tensors before any persistent-buffer allocation in
  // CalculateOpDataFullyConnected(). The per-channel FC path allocates
  // persistent multiplier metadata, and holding temp tensor buffers across
  // that allocation trips the arena allocator's temp-buffer invariants on ESP32.

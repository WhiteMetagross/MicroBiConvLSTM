...
RuntimeShape SwapRowColumnDims(const RuntimeShape& shape) {
  RuntimeShape swapped(shape.DimensionsCount());
  for (int i = 0; i < shape.DimensionsCount(); ++i) {
    swapped.SetDim(i, shape.Dims(i));
  }
  const int last = shape.DimensionsCount() - 1;
  const int second_last = shape.DimensionsCount() - 2;
  swapped.SetDim(second_last, shape.Dims(last));
  swapped.SetDim(last, shape.Dims(second_last));
  return swapped;
}
...
void TransposeRowsColumnsImpl(const TfLiteEvalTensor& tensor_in,
  ...
  const RuntimeShape input_shape = tflite::micro::GetTensorShape(&tensor_in);
  const int dims_count = input_shape.DimensionsCount();
  const int rows = input_shape.Dims(dims_count - 2);
  const int cols = input_shape.Dims(dims_count - 1);
  int batches = 1;
  for (int i = 0; i < dims_count - 2; ++i) {
    batches *= input_shape.Dims(i);
  }

  const T* input_data = tflite::micro::GetTensorData<T>(&tensor_in);
  T* output_data = tflite::micro::GetTensorData<T>(tensor_out);
  for (int batch = 0; batch < batches; ++batch) {
    const T* batch_input = input_data + (batch * rows * cols);
    T* batch_output = output_data + (batch * rows * cols);
    for (int row = 0; row < rows; ++row) {
      for (int col = 0; col < cols; ++col) {
        batch_output[col * rows + row] = batch_input[row * cols + col];
      }
    }
  }
}
...
TfLiteStatus TransposeRowsColumns(const TfLiteEvalTensor& tensor_in,
  ...
  switch (tensor_in.type) {
    ...
    case kTfLiteInt16:
      ...
      return kTfLiteOk;
    case kTfLiteInt32:
      TransposeRowsColumnsImpl<int32_t>(tensor_in, tensor_out);
      return kTfLiteOk;
    case kTfLiteInt64:
      TransposeRowsColumnsImpl<int64_t>(tensor_in, tensor_out);
      return kTfLiteOk;
    default:
      MicroPrintf("BATCH_MATMUL transpose doesn't support tensor type %s",
                  TfLiteTypeGetName(tensor_in.type));
      return kTfLiteError;
  }
}
...
TfLiteStatus ReshapeOutputTensor(TfLiteContext* context, TfLiteNode* node,
  ...
  TfLiteIntArray* new_dims = static_cast<TfLiteIntArray*>(
      context->AllocatePersistentBuffer(context,
                                        TfLiteIntArrayGetSizeInBytes(output_rank)));
  TF_LITE_ENSURE(context, new_dims != nullptr);
  new_dims->size = output_rank;
  for (int i = 0; i < output_rank - 2; ++i) {
    new_dims->data[i] =
        std::max(extended_lhs_shape.Dims(i), extended_rhs_shape.Dims(i));
  }
  new_dims->data[output_rank - 2] =
      adj_x ? extended_lhs_shape.Dims(output_rank - 1)
            : extended_lhs_shape.Dims(output_rank - 2);
  new_dims->data[output_rank - 1] =
      adj_y ? extended_rhs_shape.Dims(output_rank - 2)
            : extended_rhs_shape.Dims(output_rank - 1);
  output->dims = new_dims;
  output_eval->dims = new_dims;
  return kTfLiteOk;
}

"""
LightDeepConvLSTM Training Script

This script trains the LightDeepConvLSTM architecture on HAR datasets.
...
Usage:
    python trainLightDeepConvLSTM.py --dataset ucihar --seeds 5
    python trainLightDeepConvLSTM.py --dataset pamap2 --seeds 3 --epochs 300
    python trainLightDeepConvLSTM.py --dataset all --seeds 5 --epochs 200
"""
...
from models import LightDeepConvLSTM, createLightDeepConvLSTM
...
def loadDataset(datasetName: str, batchSize: int):
    ...
    # Import from local data folder (self-contained for LightDeepConvLSTM paper)
...
def trainModel(
    ...
    """
    Train LightDeepConvLSTM on a specific dataset with a given seed.
    ...
    if verbose:
        print(f"\n{'='*60}")
        print(f"Training LightDeepConvLSTM on {config['name']}")
    ...
    checkpointPath = os.path.join(
        saveDir, f"lightdeepconvlstm_{datasetName}_seed{seed}.pt"
    )
    torch.save({
...
def trainAllSeeds(
    ...
    """
    Train LightDeepConvLSTM on a dataset with multiple seeds.
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Train LightDeepConvLSTM on HAR datasets'
    )
    parser.add_argument(
